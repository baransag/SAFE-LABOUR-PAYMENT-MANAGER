    ...
    try {
      const res = await fetch('/api/transactions', {
        ...
        body: JSON.stringify({
          ...
          remarks: remarks.trim() || 'Advance payment given',
        }),
      });
      ...
      <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-slate-100">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-100">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-[#D4A72C]/15 text-[#D4A72C] flex items-center justify-center">
              <HandCoins className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-slate-900">+ Add Advance Given</h2>
              <p className="text-xs text-[#59718A]">Disburse advance funds (Increases Outstanding Advance)</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4">
            ...
            >
              ...
              {labours.map((l) => (
                <option key={l.id} value={l.id}>
                  {l.name} ({l.id}) - {l.workType} [Adv: Rs. {l.balances?.outstandingAdvance?.toLocaleString() || 0}]
                </option>
              ))}
            </select>
            {currentLabour && (
              <div className="mt-1.5 flex items-center justify-between text-[11px] text-[#59718A] px-1">
                ...
                <span>
                  Current Advance:{' '}
                  <strong className="text-[#D4A72C]">
                    Rs. {currentLabour.balances?.outstandingAdvance?.toLocaleString() || 0}
                  </strong>
                </span>
              </div>
            )}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-800 mb-1">
              Advance Amount (Rs.) <span className="text-[#E07A47]">*</span>
            </label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="text"
                placeholder="e.g. Cash voucher #41"
                value={reference}
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="e.g. Emergency family expense"
                value={remarks}
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-5 py-2.5 text-xs font-bold text-white bg-[#D4A72C] hover:bg-[#b89024] rounded-xl shadow-md shadow-[#D4A72C]/20 transition flex items-center gap-2 disabled:opacity-50"
            >
